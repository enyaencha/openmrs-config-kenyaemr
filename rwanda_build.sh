...
# Build assets
echo "Building Rwanda EMR 3.x assets ..."
CWD=$(pwd)
npx --legacy-peer-deps openmrs@next build \
  --build-config ./frontend-config/rwanda/build-config.json \
  --target ./frontend \
  --page-title "Rwanda EMR" \
...
npx --legacy-peer-deps openmrs@next assemble \
  --manifest \
  --mode config \
  --config ./frontend-config/rwanda/build-config.json \
  --target ./frontend

# Copy Rwanda-specific required files
cp -a "${CWD}/assets/rwanda/." "${CWD}/frontend"
cp "${CWD}/frontend-config/rwanda/rwandaemr.config.json" "${CWD}/frontend"
cp "${CWD}/frontend-config/rwanda/openmrs.config.json" "${CWD}/frontend"
