...
# Build assets
echo "Building LiberiaEMR 3.x assets ..."
CWD=$(pwd)
npx --legacy-peer-deps openmrs@next build \
  --build-config ./frontend-config/liberia/build-config.json \
  --target ./frontend \
  --page-title "LiberiaEMR" \
...
npx --legacy-peer-deps openmrs@next assemble \
  --manifest \
  --mode config \
  --config ./frontend-config/liberia/build-config.json \
  --target ./frontend

# Copy Liberia-specific required files
cp "${CWD}/assets/liberia/liberia-primary-logo.svg" "${CWD}/frontend"
cp "${CWD}/assets/liberia/liberia-login-logo.svg" "${CWD}/frontend"
cp "${CWD}/assets/liberia/liberia-login-background.svg" "${CWD}/frontend"
cp "${CWD}/assets/liberia/liberia-badge.svg" "${CWD}/frontend"
cp "${CWD}/assets/liberia/liberia-republic-logo.svg" "${CWD}/frontend"
cp "${CWD}/assets/liberia/liberia-health-logo.svg" "${CWD}/frontend"
cp "${CWD}/assets/liberia/favicon.svg" "${CWD}/frontend"
cp "${CWD}/frontend-config/liberia/liberiaemr.config.json" "${CWD}/frontend"
cp "${CWD}/frontend-config/liberia/openmrs.config.json" "${CWD}/frontend"
