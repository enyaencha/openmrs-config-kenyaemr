...
# Build assets
echo "Building DRC EMR 3.x assets ..."
CWD=$(pwd)
npx --legacy-peer-deps openmrs@next build \
  --build-config ./frontend-config/drc/build-config.json \
  --target ./frontend \
  --page-title "DRC EMR" \
...
npx --legacy-peer-deps openmrs@next assemble \
  --manifest \
  --mode config \
  --config ./frontend-config/drc/build-config.json \
  --target ./frontend

# Copy DRC-specific required files
cp -a "${CWD}/assets/drc/." "${CWD}/frontend"
cp "${CWD}/frontend-config/drc/drcemr.config.json" "${CWD}/frontend"
cp "${CWD}/frontend-config/drc/openmrs.config.json" "${CWD}/frontend"
