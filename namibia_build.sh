...
# Build assets
echo "Building Namibia EMR 3.x assets ..."
CWD=$(pwd)
npx --legacy-peer-deps openmrs@next build \
  --build-config ./frontend-config/namibia/build-config.json \
  --target ./frontend \
  --page-title "Namibia EMR" \
...
npx --legacy-peer-deps openmrs@next assemble \
  --manifest \
  --mode config \
  --config ./frontend-config/namibia/build-config.json \
  --target ./frontend

# Copy Namibia-specific required files
cp "${CWD}/assets/namibia/namibia-primary-logo.svg" "${CWD}/frontend"
cp "${CWD}/assets/namibia/namibia-login-logo.svg" "${CWD}/frontend"
cp "${CWD}/assets/namibia/namibia-login-background.jpg" "${CWD}/frontend"
cp "${CWD}/assets/namibia/namibia-badge.svg" "${CWD}/frontend"
cp "${CWD}/assets/namibia/namibia-republic-logo.svg" "${CWD}/frontend"
cp "${CWD}/assets/namibia/namibia-health-logo.svg" "${CWD}/frontend"
cp "${CWD}/assets/namibia/favicon.svg" "${CWD}/frontend"
cp "${CWD}/frontend-config/namibia/namibiaemr.config.json" "${CWD}/frontend"
cp "${CWD}/frontend-config/namibia/openmrs.config.json" "${CWD}/frontend"
